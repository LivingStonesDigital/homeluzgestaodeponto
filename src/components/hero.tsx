...
const TrustBadge = ({ number, label, sublabel }: { number: string; label: string; sublabel: string }) => (
  <div className="flex items-center gap-3">
    <div className="h-12 w-12 rounded-full bg-primary/10 flex items-center justify-center">
      <span className="text-primary font-serif font-semibold text-lg">{number}</span>
    </div>
    <div>
...
const FloatingCard = () => (
  <div className="absolute -bottom-6 -left-4 md:-left-8 bg-card p-6 rounded-xl shadow-lg border border-border max-w-[280px]">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <div className="order-2 lg:order-1">
            ...
            <div className="mt-12 flex items-center gap-8 flex-wrap">
              <TrustBadge number="10+" label="Anos de" sublabel="Experiência" />
              <TrustBadge number="50+" label="Famílias" sublabel="Atendidas" />
              <TrustBadge number="24h" label="Suporte" sublabel="Disponível" />
            </div>
          </div>
